// Mirrors the rehydration branch exactly, so both saved shapes are proven to reopen.
type V = { key?: string; name: string; human?: string; text: string };
function rehydrate(content: string) {
  try {
    const parsed = JSON.parse(content) as V[] | { question?: string; voices?: V[] };
...
let fails = 0;
const ok = (n: string, c: boolean, x?: unknown) => { if (c) console.log('  PASS  ' + n); else { fails++; console.log('  FAIL  ' + n, JSON.stringify(x)); } };

console.log('\n=== a saved council reopens ===\n');
const current = JSON.stringify({ question: 'Is this the right plan?', voices: [
...
const legacy = JSON.stringify([{ name: 'The Contrarian', text: 'x' }, { name: 'The Executor', text: 'Day 3: do it.' }]);
const b = rehydrate(legacy)!;
