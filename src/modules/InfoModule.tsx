    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        <article className="max-w-[720px] mx-auto px-7 py-10 pb-24">
          ...
          <CmdTable rows={[
            ['/leads', 'Find leads (guided)', 'Opens a card where you set exactly what you want — company size, city, sector, seniority, how many — and whether every lead must have a real LinkedIn profile or a phone/email. Those are applied as filters on the result, so what comes back matches what you asked for. Every lead is then verified: the profile is opened and confirmed, and phone and email are read from Google Maps and the company site. The same card can top up a list you already have, skipping everyone already on it, so you never get the same person twice. When it finishes, one button sends those leads straight to the outreach copilot.', 'Type /leads, pick 11–50 employees + Bangalore + Founder, then “Send to outreach”'],
          ...
          <CmdTable rows={[
            ...
            ['/outreach', 'Send outreach (copilot)', 'Works from BOTH your LinkedIn connections and the lead lists in your Brain, each tagged so you can see who is who and filter to either group. For someone you are already connected to it writes a full message. For a lead you are not connected to it writes a connection-request note instead (LinkedIn only lets you message people you are connected to), and the full message is written automatically the moment they accept. Skips anyone you have already messaged.', 'Type /outreach, or press “Send to outreach” on a finished lead list'],
            ['—', 'Check who accepted', 'In the copilot, “Check pending” settles every outstanding connection request in one pass against LinkedIn’s own sent-invitations list: accepted (you can message them now), still waiting (with how long it has been), or expired and declined — which go back on your to-do list. If LinkedIn cannot be read it changes nothing and says so, rather than guessing.', 'Open the copilot and press “Check pending”'],
            ...
            ['/refine', 'Refine outreach messages', 'Rewrites the messages already in the copilot to be more personal to each person — from their profile and what you’re selling. Add a note on how you want them (“warmer”, “lead with the time we save them”) and it follows it. Only touches people you haven’t messaged yet; nothing is sent.', 'Refine the outreach messages — warmer, and lead with our local-first angle'],
