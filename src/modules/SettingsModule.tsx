...
const WHATS_NEW: { version: string; items: string[] } = {
  version: APP_VERSION,
  items: [
    "Your attached files are no longer thrown away when an answer has to be retried. If the model returned nothing, the app tried again -- but rebuilt the request from what is shown on screen, where an attachment is only its name. So it asked for notes on four lecture decks it had never been given, and was told, correctly, \"since you did not attach the PPT files I cannot generate the specific content\", followed by a blank template. The retry now carries everything the first attempt had.",
    "A model that returns nothing is replaced instead of reported. The app already knows which models on your key actually answer and how big each one is; it just was not reaching for the next one. An empty reply on a long request now moves up to a larger model on the same key, carries on, and the chat stays on the model that worked. Nothing extra is charged.",
    "The microphone no longer sends the owner to a pricing page. Every screen that shows your plan treats a head or admin account as Enterprise, but the feature checks did not -- so the person who runs adris was gated out of voice input in their own product.",
    "The upgrade window inside the app was showing the wrong prices. It offered Business at ₹9,999 and Growth at ₹19,999 while the website sold them at ₹6,499 and ₹12,999, and did not know Starter existed at all. The figures now come from the same place the pricing page is built from, so they cannot drift apart again. It also advertised single sign-on, which is not built; that came off the website weeks ago and has now come out of here too.",
    "The plan cards read properly. Allowances said \"~4,000k tokens\" where they meant 4M, an old figure sat crossed out beside the new one over three wrapped lines, and the four cards each put their heading at a different height because the one with the least text centred itself lowest.",
  ]
};
