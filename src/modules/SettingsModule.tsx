...
const WHATS_NEW: { version: string; items: string[] } = {
  version: APP_VERSION,
  items: [
    "The PowerPoint file now contains the whole deck. The chat deck can build seventeen kinds of slide; the PowerPoint writer only knew how to draw eight of them, and the other nine arrived carrying their heading and nothing else. A chart slide reached PowerPoint with no numbers, a pricing slide with no plans, a timeline with no dates, a team slide with no people. The slide COUNT was always right, which is why it looked like a design problem rather than missing content. Measured on a deck using every layout: 35% of everything on it never reached the file. It is now 0%.",
    "Words are sized to the slide they are on. Every size in the PowerPoint file was a fixed number, so a long heading was set at the same size as a short one and ran off the edge -- the same deck that fitted perfectly in the chat, where it is measured and shrunk to fit. It is now measured for PowerPoint too, before the file is written.",
    "Charts are real charts. A chart slide used to be dropped entirely; it is now a proper PowerPoint chart you can click, recolour and edit like any other, not a picture of one.",
    "The long dash is gone. It is the clearest sign that text was written by a machine, and the app was adding one itself in front of every quotation. Ranges like 2020-2024 keep their tight form.",
    "The file no longer announces how it was made. Every deck carried \"PptxGenJS Presentation\" as its subject and \"adris.tech\" as its author, both visible in File then Info, two clicks from a slide you were about to present as your own work. The subject is now the deck's own title and the author is you. Section dividers no longer have the word SECTION stamped across them either; they are numbered, the way a designer numbers them.",
    "Slides no longer stretch or huddle. One pricing plan used to become a full-width box with a line of text adrift inside it, and a three-point timeline crowded into the top with the bottom half of the slide empty. Panels are now capped and centred.",
    "You do not have to guess how many slides you need. The setup box demanded a number between 4 and 24 before it would start. There is now a \"Decide for me\" option, and it is the default: it reads your notes and files and picks the length from the material. The dial is still there if you want a specific number, and it goes further than 24 now.",
    "The number of slides you are told is the number you get. It said 31 while it worked and then built 29, because the number it announced was the target and the review pass afterwards is allowed to merge slides that say the same thing. Both are right; saying nothing about the difference was not. It now tells you the real count and why it changed.",
  ]
};
