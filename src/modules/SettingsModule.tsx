...
const WHATS_NEW: { version: string; items: string[] } = {
  version: APP_VERSION,
  items: [
    "Opening a chat that contained a presentation no longer hangs the app. A saved deck is stored as its finished HTML with the whole design embedded inside it, so every picture is held twice -- a thirty-slide deck carrying five figures out of a Word document is 6.7 MB. That was being re-read from scratch on every single repaint, which rebuilt the deck and made the preview reload 6.7 MB of HTML and five half-megabyte images, over and over. It is now read once. A deck you had just made was never affected; only one you came back to, which is exactly when you saw it.",
    "It runs on the source you picked, or it tells you it could not. Choosing your own API key or a local model quietly handed the work to adris.tech whenever the check for that key failed -- and the check needs both the app and the network, so a dropped connection was enough. The title bar went on showing your choice while something else answered and spent your adris.tech allowance. A failed check is no longer treated as proof the key is gone, and if your choice really is unavailable the chat says so instead of switching in silence.",
    "\"I cannot browse the internet\" is caught and sent back. Asked three times to research a page whose link was pasted in, the agent replied that it could not browse the live internet and that its knowledge cutoff stopped it seeing the page -- then produced a long, confident answer entirely from memory. The page opens fine, and the app reads all 175 companies off it. Denials of that kind now go back to be done properly. Previously the check was switched off the moment any tool had run, which is precisely when this happens.",
    "Presentations end properly instead of stopping. On a long deck the model is still writing slides when it runs out of room, so the last thing it writes is whatever slide it was on. The deck now always closes: a recap built from its own headings, then a closing slide, added only when they are missing. A deck that already ends well is left exactly as it is.",
    "The chat deck and the PowerPoint file now show the same thing, and that is checked on every build. A slide carrying both a picture and an intro line lost that line in the chat while keeping it in PowerPoint. All seventeen layouts are now compared between the two renderers automatically.",
    "\"Polishing 34 of 30 slides\" made sense to nobody. The review pass is allowed to split a slide that is carrying too much, so it can genuinely produce more slides than it was given. The count was right; the sentence was wrong.",
  ]
};
