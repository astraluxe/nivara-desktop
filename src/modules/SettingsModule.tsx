...
const WHATS_NEW: { version: string; items: string[] } = {
  version: APP_VERSION,
  items: [
    "Picking one of the offered options now actually replies. When an agent ends by offering you things to do next, that card says \"Tap one, then confirm - it is sent as your reply\". It was not being sent. The text was saved and then drawn back inside the card as though the agent had said it, so you picked \"explain this in more detail\", saw your own request reflected at you, and no answer ever came. It is sent for real now, and the reply arrives in the conversation like any other message.",
    "An options block the model did not finish writing no longer spills onto the screen. If it ran out of room mid-block, the raw code was left at the end of your answer - a wall of brackets and quotes where the options should have been. It is removed and your answer is left clean.",
    "The title bar names what is actually running, and shows the model with it. Checking which key you have needs the app, a look at your machine and a sign-in check, and until all three answer there was nothing to go on - so someone with a key connected was told \"Connect your own key\" on every launch, and the wrong source could be named entirely. Not having checked yet is no longer treated as proof your key is gone. The model it is thinking with now sits beside the key, because whose key you are using and what it is running are two different questions.",
    "Pictures from your own files can now appear in the answer. Attach a document with diagrams and ask about them: when the picture is the point - a diagram being explained, a chart whose numbers are being quoted, a screenshot of the thing that is wrong - it is placed in the reply where it belongs, instead of telling you to go back to the file and look. If a reference could mean more than one picture it shows none, because a confidently wrong diagram under a sentence is worse than no diagram.",
  ]
};
